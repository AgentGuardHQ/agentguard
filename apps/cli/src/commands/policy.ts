...
function formatSuggestionsAsYaml(
  ...
  const lines: string[] = ['# AgentGuard policy suggestions (auto-generated by `agentguard policy suggest`)', 'rules:'];
  for (const s of suggestions) {
    if (s.type === 'allow_rule' || s.type === 'scope_expansion') {
      ...
      lines.push(`  # threshold_adjustment for ${s.actionType} — confidence ${(s.confidence * 100).toFixed(0)}%`);
      lines.push(`  # ${s.description.replace(/\n/g, ' ')}`);
    }
  }
  return lines.join('\n') + '\n';
...
function renderSuggestTerminal(
  ...
  process.stderr.write(`  Analyzed ${patterns.length} denial pattern(s) across ${sessionCount} session(s)\n\n`);

  if (suggestions.length === 0) {
    process.stderr.write(`  ${dim('No actionable suggestions at current confidence thresholds.')}\n\n`);
    return;
  }
  ...
  for (const s of suggestions) {
    const confidence = (s.confidence * 100).toFixed(0);
    const typeLabel =
      s.type === 'allow_rule' ? color('allow_rule', 'green')
      : s.type === 'scope_expansion' ? color('scope_expansion', 'yellow')
      : s.type === 'threshold_adjustment' ? color('threshold_adjustment', 'cyan')
      : dim(s.type);
...
async function policySuggest(args: string[]): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(`  Run \`agentguard guard --store sqlite\` to start recording sessions.\n`);
    process.stderr.write(`  Session data is stored in: ${baseDir}/\n\n`);
    process.stderr.write(
      `  ${dim('Advanced cloud suggestions: https://agentguard.dev')}\n\n`
    );
    return 0;
  }
    ...
    .map((e) => ({
      ...
      reason:
        typeof e.reason === 'string'
          ? e.reason
          : (e.policyRule ?? e.invariant ?? 'denied'),
  ...
  if (jsonOutput) {
    process.stdout.write(
      JSON.stringify(
        { patterns: filteredPatterns, suggestions: filteredSuggestions, sessionCount: distinctSessions },
        null,
        2
      ) + '\n'
    );
