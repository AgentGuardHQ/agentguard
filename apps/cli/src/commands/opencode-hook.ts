// AgentGuard OpenCode hook — before-tool governance + after-tool error monitoring.
...
interface OpenCodeSessionState extends Record<string, unknown> {
  formatPass?: boolean;
  testsPass?: boolean;
  writtenFiles?: string[];
}
...
function sessionStatePath(sessionId: string): string {
  return join(tmpdir(), 'agentguard', `opencode-session-${sessionId}.json`);
}

function readSessionState(sessionId: string | undefined): OpenCodeSessionState {
  const key = sessionId || String(process.ppid) || 'default';
  try {
    return JSON.parse(readFileSync(sessionStatePath(key), 'utf8')) as OpenCodeSessionState;
  } catch {
    return {};
  }
}
...
function writeSessionState(
  ...
  patch: Partial<OpenCodeSessionState>
): void {
  const key = sessionId || String(process.ppid) || 'default';
  try {
...
/**
 * Extract the target file path from an OpenCode hook payload for path-aware policy resolution.
 */
function extractTargetPath(payload: OpenCodeHookPayload): string | undefined {
  ...
  if (payload.tool === 'shell' && typeof input.command === 'string') {
...
export async function openCodeHook(hookType?: string, extraArgs: string[] = []): Promise<void> {
  loadProjectEnv();
  ...
  try {
    ...
    if (isBeforeHook) {
      const payload = parseOpenCodePayload(data);
...
/** Parse raw JSON data into OpenCodeHookPayload. */
function parseOpenCodePayload(data: Record<string, unknown>): OpenCodeHookPayload {
  const sessionId =
    (data.sessionId as string | undefined) || process.env.OPENCODE_SESSION_ID || undefined;
...
async function handleBeforeHook(
  ...
  const { processOpenCodeHook, formatOpenCodeHookResponse } = await import('@red-codes/adapters');
  ...
  try {
    ...
    if (telemetryMode !== 'off') {
      ...
      cloudSinks = await createCloudSinks({
        ...
        agentId: resolveAgentIdentity() ?? 'opencode',
  ...
  if (storage?.sessions) {
    storage.sessions.start(sessionKey, 'opencode-hook', {
  ...
  const result = await processOpenCodeHook(kernel, payload, enrichedState, resolvedPersona);
  kernel.shutdown();
  ...
  if (!result.allowed) {
    ...
    if (shouldEnforce) {
      const response = formatOpenCodeHookResponse(result);
      if (response) {
...
// NOTE: No direct entry point here. All invocations go through
// the CLI router in bin.ts: `case "opencode-hook": openCodeHook(args[1], ...)`
