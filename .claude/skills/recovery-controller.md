...
2. If no triage issue exists, create one:
   ...
   *Auto-created by recovery-controller on $(date -u +%Y-%m-%dT%H:%M:%SZ)*" \
     --label "priority:critical" --label "source:recovery-controller" --label "status:pending"
   ```
...
3. Create an alert issue if the agent has been silent >72h:
   ...
   **Impact:** <what stops working when this agent is down>
   ...
   ---
   *Auto-created by recovery-controller on $(date -u +%Y-%m-%dT%H:%M:%SZ)*" \
     --label "source:recovery-controller" --label "priority:high" --label "status:pending"
   ```
...
```bash
gh issue list --state open --label "source:recovery-controller" --label "status:pending" --json number --jq '.[0].number' 2>/dev/null
```
...
gh issue create \
  ...
  --label "source:recovery-controller" --label "status:pending"
```
